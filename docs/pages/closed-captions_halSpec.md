...
This interface is not required to be involved in any power management functionality.
...
3. When the closed caption data is no longer needed, stop caption decoding using `closedCaption_stop()`. This will stop the `HAL` callbacks.
4. Start and stop of decoding is notified to the `caller` using `closedCaption_decodeCallback()`.
